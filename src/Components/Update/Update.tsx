...
export const Update: React.FC<PropTypes.InferProps<typeof updatePropTypes>> = ({
  children,
}) => {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        const hasNewerVersion = compareVersions.compare(
          pkg.version,
          metadataResponse.currentVersion,
          '<',
        );
  ...
  return versions.requiresNewerVersion ? (
    <SafeAreaView style={styles.container}>
      ...
      <Image style={styles.image} source={require('~/Assets/launch.png')} />

      <Text style={styles.header}>Common just got better</Text>

      <Text style={styles.description}>
        A new version of the Common app is available. Update now to get the
        latest feature updates and bug fixes.
      </Text>
        ...
        onPress={() => {
          if (Platform.OS === 'ios') {
            Config.ENV === 'staging'
              ...
              : Linking.openURL(
                  `itms-apps://itunes.apple.com/us/app/apple-store/${appId}`,
                );
