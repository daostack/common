...
const CommonMembersList = ({navigation, members, commonId, limit, horizontal, bottomSheetStore}) => {
  ...
  const showUserProfile = (uid, userInfo) => {
    navigation.navigate('Profile', {userId: uid, userInfo});
  };
    ...
    <View style={horizontal && {...layout.flexRow, paddingLeft: (limitedMembers.length - 1) * 15}}>
      {membersInfo ? (
        membersInfo.map((member, i) => (
          horizontal
            ? (
              <TouchableOpacity style={{position: 'relative', left: i * -15}} onPress={() => showUserProfile(member.uid, member) } key={`touch_${i}`}>
                <MemberImage
                  id={i}
                  userInfo={member}
            ...
            : (
              <TouchableOpacity style={styles.item} onPress={ () => showUserProfile(member.uid, member) } key={`touch_${i}`}>
                <MemberCard
                  key={i}
                  userInfo={member}
                />
