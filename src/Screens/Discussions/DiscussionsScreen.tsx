...
import {ModerationFormStore, ModerationFormFields} from '~/Stores/FormStores';
...
import {NewDiscussionMessage} from '~/Stores/Models/new-discussion-message';
const {width} = Dimensions.get('window');

export type DiscussionRouteProps = {
  params: {
    commonId: string;
    discussionId: string;
...
const DiscussionsScreen: React.FC<{}> = () => {
  ...
  const discussion = discussionStore.getDiscussionById(discussionId);

  const user = discussion?.ownerId
    ? userStore.getUserById(discussion!.ownerId)
    : null;
  const common = commonStore.getCommonById(commonId);
  const hasPermission = common.getPermission();
  const isMember = common.isUserMember;
  const [newMessage] = React.useState(new NewDiscussionMessage(discussion));
      ...
      {discussion.images ? (
        <ScrollView
          ...
          <View style={styles.imageGallery}>
            <View style={{width: 20}} />
            {discussion.images.map((currImage, currIndex) => (
                ...
                <View
                  ...
                  <View style={{flex: 1, paddingHorizontal: 10}}>
                    ...
                    <Text style={styles.date}>
                      {moment(discussion.createTime.toDate()).fromNow()}
                    </Text>
                  </View>
                </View>
  ...
  return (
    <SafeAreaView style={styles.safeView}>
      ...
      <ImageView
        images={
          discussion.images
            ? discussion.images.map((x) => ({uri: x.value}))
            : []
        }
...
export default observer(DiscussionsScreen);
