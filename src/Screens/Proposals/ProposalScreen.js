...
import {
  text, layout, colors, sizeM, sizeS, sizeXS, font,
} from '../../Theme';
...
import ProposalService, { PROPOSAL_STAGES_ACTIVE, PROPOSAL_TYPE } from '../../Services/ProposalService';
...
const ProposalScreen = ({
  navigation, route, userStore, bottomSheetStore, props,
}) => {
  ...
  const renderVoting = proposalInfo
    ...
    if (isVoteByYou) {
      ...
      return (
        <View style={{ ...layout.content, ...layout.flexRow, ...{ padding: 0 } }}>
          ...
          />
          <Text style={{ ...styles.votedByYouText, ...{ color } }}>
            {message}
          </Text>
        </View>
      );
  ...
  const renderVotingButtons = (refference) => (
    <View ref={refference} style={{ ...layout.content, padding: 0, width: '100%' }}>
  ...
  const headerContainerStyle = proposalInfo.type === PROPOSAL_TYPE.FundingRequest
    ? {
      ...layout.content,
      ...layout.flexStart,
      ...{ paddingBottom: 0 },
  ...
  if (proposalInfo) {
    progressBarWidthPercent = (proposalInfo.votesFor
  ...
  return (
    <>
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: colors.white }}>
        {showStickyTabBar && (
        <View style={{
          position: 'absolute', top: 0, width: '100%', paddingBottom: 5, zIndex: 999,
        }}
        >
