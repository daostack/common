...
export interface FormStoreValidation {
  name: string;
  formStore: FormStore;
  validateRule: string | Record<string, any>;
  multiName: string;
...
export class FormStore {
  ...
  multiFieldsByValidatorKey: Record<string, any> = {};
  ...
  getFormField = (name: string, multiName?: string) => {
    if (multiName) {
      const multiIndexInfo = name.split('_');
      const currMultiIndex = parseInt(multiIndexInfo[0]);
  ...
  registerValidationRule({
    ...
    Validator.register(ruleName, validateFunc, errorMessage);
  }
  ...
  registerFormField(
    ...
    let currValue = {
      ...
      rule: validateRule,
      changed: false,
      blurredAtLeastOnce: immediateValidation,
    };
    ...
    if (multiName) {
      ...
      if (currMultiValueField) {
        const currMultiValue = this.form.fields[multiName]
  ...
  removeFormField(name: string, multiIndex?: number) {
    ...
    if (multiIndex === 0 || multiIndex) {
      if (this.form.fields[name] && this.form.fields[name][multiIndex]) {
        const newFormFieldObj: string[] = [];
        this.getFormField(name).forEach(
          (currKey: string, currIndex: number) => {
  ...
  fieldBlurred = (name: string, multiName: string) => {
    this.getFormField(name, multiName).blurredAtLeastOnce = true;
  ...
  fieldChanged = (
    name: string,
    value: any,
    _triggerValidation: boolean = false,
    multiName?: string,
  ) => {
    this.getFormField(name, multiName).value = value;
    if (this.getFormField(name, multiName).blurredAtLeastOnce) {
      this.validateField(name, multiName);
    }
  ...
  getFormFieldsJson = (onlyChangedFields = false) => {
    let changedFieldsJson: Record<string, any> = {};

    for (const key in this.form.fields) {
      const formField = this.form.fields[key];
      let currValue: Record<string, any> = [];

      if (Array.isArray(formField)) {
        formField.forEach((currMultiFormField) => {
          const multiFieldValue: Record<string, any> = {};
          const nextIndex: string | number = currValue.length;
          // Multi Links
          if (typeof currMultiFormField.value === 'object') {
            Object.keys(currMultiFormField).forEach((currKey) => {
              ...
              if (
                currMultiFormField[currKey].value &&
                currMultiFormField[currKey].value.length > 0
              ) {
                if (onlyChangedFields) {
                  if (currMultiFormField[currKey].changed) {
                    multiFieldValue[currKey] =
                      currMultiFormField[currKey].value;
                  }
                } else {
  ...
  // Private functions
  validateField = (name: string, multiName?: string) => {
  ...
  getValidator = (name?: string, multiName?: string) => {
  ...
  getValidatorParams = (fieldName?: string, multiField?: string) => {
    const fieldsData: Record<string, any> = {};
    const fieldsRule: Record<string, any> = {};
