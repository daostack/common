...
export const validPassport = {
  ...
  validateFunc: (value: any) => isValidId(value), // || isValidPassport(value),
...
export const firstLastNameValidate = {
  ...
  validateFunc: (value: string) => new RegExp(/^\w+(?:\s\w+)+$/).test(value),
  errorMessage:
...
export const latinOnly = {
  ...
  validateFunc: (value: string) => new RegExp(/^[a-zA-Z'’. ]*$/).test(value),
